...
\begin{figure}[!hbtp]
  \centering
	\begin{minipage}[!hbtp]{0.48\columnwidth}
		\centering
    \bisubcaptionbox{实验中激励源振幅序列\label{fig:nonlinearity_1}}{Sequence of excitation source amplitudes in the experiment.}
		[7cm]{\includegraphics[height=5.2cm]{3_nonlinearity_protocol.png}}
		\\
    \bisubcaptionbox{声速相对变化值 $\Delta c/c$ 和相似性参数 $\Psi_{i,i+1}$ 变化\label{fig:nonlinearity_2}}{Relative change in sound speed $\Delta c/c$ and change in similarity parameter $\Psi_{i,i+1}$.}
    [7cm]{\includegraphics[height=5.2cm]{3_nonlinearity_vsound_psi.png}}
	\end{minipage}
	\begin{minipage}[!hbtp]{0.48\columnwidth}
		\centering
    \bisubcaptionbox{响应信号基波分量与激励源振幅线性关系的破坏\label{fig:nonlinearity_3}}{Failure of linear relationship between response signal fundamental component and excitation source amplitude.}
		[7cm]{\includegraphics[height=5.2cm]{3_nonlinearity_1w.png}}
		\\
    \bisubcaptionbox{响应信号一次谐波分量与激励源振幅平方线性关系的破坏\label{fig:nonlinearity_4}}{Failure of linear relationship between response signal first harmonic component and the square of excitation source amplitude.}
    [7cm]{\includegraphics[height=5.2cm]{3_nonlinearity_2w.png}}
	\end{minipage}
	\bicaption{在单轴应力 P = 38.6 \unit{\kilo\pascal}、采用连续激励协议下，超声响应信号的声速、相似性参数、谐波线性分析}{Sound velocity, similarity parameters, and harmonic linearity analysis of ultrasonic response signals under uniaxial stress P = 38.6 \unit{\kilo\pascal}, applying a continuous excitation protocol}\label{fig:nonlinearity}
\end{figure}
...
于是我们观察到了相似性参数在振幅约为 9.14 \unit{\volt} 和在泵浦结束时的首个试探信号都出现了骤降，随后相似性参数均趋近于 1，说明颗粒介质内部结构因为外部声学激励而发生了变化；声速的相对变化计算来源于小振幅时所测声速的平均值，而在逐渐激励的过程中有逐渐降低的趋势，这说明了颗粒介质的模量因为连续激励而有所降低。在小于约 3.95 \unit{\volt} 时，基波和一次谐波都分别与激励振幅的一次和二次呈现良好的线性符合，而在大于该阈值后出现了偏离，这也能说明声学激励对于颗粒介质的衰减系数造成了影响。对二次谐波的分析也有过尝试，但是由于信噪比等原因效果并不理想。
